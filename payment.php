...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $paymentProof = $_FILES['paymentProof']['tmp_name'];
    $paymentProofName = $_FILES['paymentProof']['name'];

    $paymentProofData = file_get_contents($paymentProof);

    // Prepare and bind the SQL query with placeholders
    $stmt = $conn->prepare("UPDATE files_table SET payment_proof_file_name = ?, payment_proof = ? WHERE id = ?");
    $stmt->bind_param("ssi", $paymentProofName, $paymentProofData, $id);
    ...
    if ($stmt->execute()) {
        ...
        echo "Error uploading payment proof: " . $stmt->error;
    }
...
else if($_POST['mode']=="form")
{
  ...
  $paymentProofName = $_FILES['paymentProof']['name'];
      ...
      $stmt = $conn->prepare("INSERT INTO forms_table (name, email, form,number_of_copies, payment_proof_file_name , payment_proof) VALUES (?, ?, ?, ?, ?,?)");
    $stmt->bind_param("ssssss", $username, $email, $formName, $copies, $paymentProofName, $paymentProofData);

    // Read the file data and bind it to the query parameters
    $paymentProofData = file_get_contents($paymentProof);
    $stmt->send_long_data(5, $paymentProofData);
...
else if($_POST['mode']=="record")
{
  ...
  $paymentProofData = file_get_contents($paymentProof);
    ...
    $stmt->bind_param("ssiiss", $username, $email, $books,$numberOfPages,$paymentProofName, $paymentProofData);
...
margin-bottom: unset;
 ...
 <div class = "body2  container2 p-4">
        <div id="qr-code">
          <!-- Replace 'YOUR_QR_CODE_IMAGE_URL' with the actual QR code image URL -->
          <img src="manasaqr.jpg" alt="QR Code" width="400" height="400">
        </div>
        <div id="payment-form">
...
<form method="POST" enctype="multipart/form-data">
    ...
    <div class="form-group">
        ...
        <input type="text" name="copies" style="display: none;" id="copies">
        <input type="text" name="formName" style="display: none;" id="formName">
        <input type="text" name="mode" style="display: none;" id="mode">
        <input type="text" name="numberOfPages" style="display: none;" id="numberOfPages">
        <input type="text" name="books" style="display: none;" id="books">
    </div>
...
<script src="https://cdnjs.cloudflare.com/ajax/libs/bootstrap/5.3.0/js/bootstrap.bundle.min.js"></script>
  <script>
    ...
    if(mode==="form")
    {
      const copies=localStorage.getItem('copies');
