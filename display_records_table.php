...
<nav class="navbar navbar-expand-lg navbar-light bg-white fixed-top">
  ...
  <div class="body1 extraclass">
    <div class="container cards-container">
        ...
        // Retrieve all data from the records_table
        $sql = "SELECT * FROM records_table";
        $result = $conn->query($sql);

        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                // Get the data fields from the row (replace 'field_name' with actual field names)
                $name = $row['name'];
                $email = $row['email'];
                $numberOfBooks = $row['number_of_books'];
                ...
                echo '<div class="card">';
                echo "<h2>Record for: $name</h2>";
                echo "<p>Email: $email</p>";
                echo "<p>Number of Books: $numberOfBooks</p>";
                echo "<p>Number of Pages: $numberOfPages</p>";
                echo "<div class='card-buttons'>";
                echo "<button class='btn view-btn' onclick='viewFile($fileId)'>paymentproof</button>";
                echo "<button class='btn send-mail-btn' onclick='sendEmail($fileId, \"$email\")'>Send Mail</button>";
    ...
    <script>
        function viewFile(rowId) {
            window.location.href = "records/view_payment_proof.php?id=" + rowId;
        }

        function confirmDelete(rowId) {
            if (confirm("Are you sure you want to delete this row?")) {
                window.location.href = "records/delete_row.php?id=" + rowId;
            }
        }

        function sendEmail(rowId, email) {
            if (confirm("Are you sure you want to mark this work as done and send an email?")) {
                window.location.href = "records/send_email.php?id=" + rowId + "&email=" + email;
            }
        }
    </script>
